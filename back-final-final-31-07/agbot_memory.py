...
class AgBotMemory(JsonReaderWriter):
    ...
    def add_plant(self, plant_name, 
        ...
        #Logic is now in the frontend, simply adding one to the size of the plant_set.
        ...
        print("After data, before saving")
        self.save()
        print("After safe")

    def add_mission(self, mission_name, hour, minute, action, mission_id):
        mission = {}
    ...
    def remove_plant_from_mission(self, plant_id, mission_id):
        ...
        if plant is not None:
            ...
            for mission in self.data["missions"]:
                if mission['mission_id'] == mission_id:
                    print("Mission found: ", mission['mission_name'])
                    if plant in mission['locations']:
                        print("Plant found in mission")
    ...
    def delete_mission(self, mission_id):
        for index, mission in enumerate(self.data["missions"]):
            if mission['mission_id'] == mission_id:
                ...
                self.save()
                print(f"Mission with id {mission_id} deleted")
                return
    ...
    def delete_plant(self, plant_id):
        for plant in self.data["plants"].keys():
            ...
            if self.get_plant(plant)["id"] == plant_id:
                ...
                #locations_size = len(self.get_missions())
                '''
                #Este location size esta mal... falta meterse en las misiones!!
                if locations_size > 0:
                    for index in len(self.get_missions()):
                        for location in len(self.get_missions()[index]["locations"]):
                            if self.get_missions()[index]["locations"][location] == plant:
                                print(f"Deleting {plant} from {self.get_missions()[index]['mission_name']}")
                                self.data["missions"][index]["locations"].pop(location)'''
                for mission in self.data["missions"]:
                    self.remove_plant_from_mission(plant_id, mission['mission_id'])
...
if __name__ == "__main__":
    ...
    for mission in agm.get_missions():
        print(mission)
